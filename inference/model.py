...
def create_modules(module_defs, img_size, cfg):
    ...
    _ = module_defs.pop(0)  # cfg training hyperparams (unused) - remove module [net] trong file cfg
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            elif mdef['activation'] == 'mish':      # trong yolov4 chi dung "mish"
        ...
        #         modules = avgpool
            ...
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = FeatureConcat(layers=layers)      # concat nhung layers trong route 
        ...
        #     modules = FeatureConcat3(layers=layers)
            ...
            filters = output_filters[-1]
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = WeightedFeatureFusion(layers=layers, weight='weights_type' in mdef)   # shortcut 
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.module_defs = parse_model_cfg(cfg)     # doc file cfg tra ve list cua nhung classes la tat ca cac module co trong model
...
if __name__ == '__main__':
    import torch
    model = Darknet(cfg='../cfg/yolov4.cfg')
