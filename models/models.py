...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.module_defs = parse_model_cfg(cfg)     # tra ve mot 'List' cac classes chua thong so model
        self.module_list, self.routs = create_modules(self.module_defs, img_size, cfg)
        # self.module_list: la list cac child module (la cac layer), self.routs: ds cac layer co ket noi voi cac lop sau [True, False]
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            out.append(x if self.routs[i] else [])      # neu layer i co route thi append vao out (co the de concat voi cac layer sau)
            if verbose:
    ...
    def info(self, verbose=False):
        torch_utils.model_info(self, verbose)   # print model summary
